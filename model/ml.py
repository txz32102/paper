...
# Load the dataset
df = pd.read_csv("/content/drive/MyDrive/paper/data/drugminer/esm2_320_dimensions_with_labels_original.csv")
...
xgb_metrics, xgb_y_predict = evaluate_classifier(xgb.XGBClassifier(use_label_encoder=False, eval_metric="logloss"), X_train, y_train, X_test, y_test)
print_metrics("XGBoost", xgb_metrics)

nb_metrics, nb_y_predict = evaluate_classifier(GaussianNB(), X_train, y_train, X_test, y_test)
print_metrics("Gaussian Naive Bayes", nb_metrics)

rf_metrics, rf_y_predict = evaluate_classifier(RandomForestClassifier(n_estimators=100, random_state=42), X_train, y_train, X_test, y_test)
print_metrics("Random Forest", rf_metrics)

svm_metrics, svm_y_predict = evaluate_classifier(SVC(probability=True, random_state=42), X_train, y_train, X_test, y_test)
