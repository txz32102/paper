...
def main():
    ...
    negative_test_path = (
        "data/pharos/fastadata/Independent_Test/negative_test_sequence.fasta"
    )
    positive_test_path = (
        "data/pharos/fastadata/Independent_Test/positive_test_sequence.fasta"
    )
    negative_train_path = "data/pharos/fastadata/Train/negative_train_sequence.fasta"
    positive_train_path = "data/pharos/fastadata/Train/positive_train_sequence.fasta"
    ...
    for id, sequence in negative_test:
        ...
        df.loc[len(df)] = row
    negative_train = get_negative_peptide_sequence_list(negative_train_path)
    for id, sequence in negative_train:
        row = [id, sequence, 0, "train"]
        df.loc[len(df)] = row
    positive_train = get_positive_peptide_sequence_list(positive_train_path)
    for id, sequence in positive_train:
        row = [id, sequence, 1, "train"]
        df.loc[len(df)] = row
    positive_test = get_positive_peptide_sequence_list(positive_test_path)
